...
BEGIN_MESSAGE_MAP(CEntornVGIView, CView)
	...
	// PracticaVGI --------
	ON_COMMAND(ID_PRACTICA_COTXE, &CEntornVGIView::OnPracticaCotxe)
	ON_UPDATE_COMMAND_UI(ID_PRACTICA_COTXE, &CEntornVGIView::OnUpdatePracticaCotxe)
...
// Entorn VGI: Variables de control per les opcions de menú Projecció, Objecte
	/*projeccio = CAP;*/projeccio = PERSPECT;	objecte = CAP;
...
void CEntornVGIView::dibuixa_Escena() {
	if (escena == ESCENA_PRACTICA_COTXE_1)
	{
		//dibuixa_EscenaCotxe(&mainOGLObject, col_obj, true, sw_material);
		practicaCotxe.Draw();
	}
	else
	{
...
// Inicialitzacio escena cotxe
void CEntornVGIView::OnPracticaCotxe()
{
	if (escena != ESCENA_PRACTICA_COTXE_1)
	{
...
void CEntornVGIView::OnUpdatePracticaCotxe(CCmdUI *pCmdUI)
{
	// TODO: Agregue aquí su código de controlador de IU para actualización de comandos
	if (escena == ESCENA_PRACTICA_COTXE_1) pCmdUI->SetCheck(1);
	else
	{
		pCmdUI->SetCheck(0);
	}
